# Kunden aus SEA (automatisch gespiegelt)

> Automatisch aus zentrale/data/kunden.json erzeugt, bei jeder Aenderung in SEA.
> Nicht von Hand bearbeiten. Kuratierte Registry bleibt kunden/UEBERSICHT.md.

Stand: 2026-07-21 | Anzahl: 7

| Name | Branche | Status | Aktualisiert |
|---|---|---|---|
| SwissCare | Serum/CRM, internes Produkt (Kunde 0) | In Bearbeitung | 2026-07-21 |
| Finelli | Streetwear Zürich, Lager-App auf Shopify | In Bearbeitung | 2026-07-20 |
| GzF (Gut zu Fuss) | Orthopädie Rorschach, Kundendatenbank + Mail-Flows | Noch nicht angefangen | 2026-07-20 |
| Yousef | Broker | Noch nicht angefangen | 2026-07-21 |
| Saad | Bookkeeping & Investment | Noch nicht angefangen | 2026-07-21 |
| Saif | Anwaltskanzlei | Noch nicht angefangen | 2026-07-21 |
| Salman | Schönheitschirurg | Noch nicht angefangen | 2026-07-21 |

## Notizen

### SwissCare
Läuft produktiv, wird laufend betreut. Repo auf SA-Rechner (C:/dev/swisscare-crm). Daten-Wand beachten: nie mit ai-firma-Daten mischen.

### Finelli
Angebot V6 überreicht 19.07. Deal 2500 + 1000/Mt. Produktions-Repo finelli-lagerverwaltung (Phase 1, noch kein Code). Verkaufs-Demo Cockpit auf SA-Rechner.

### GzF (Gut zu Fuss)
Phase 0-1. Budget-Deckel 850/Mt. Schmerz-Gespräch ausstehend, Inhaber Farhad Akbarzada. Noch kein Repo.

### Yousef
Akuidierung von Kunden

### Saad
Pakistan first lead

### Saif
Kontakt in Pakistan

### Salman
Connections zu anderen kunden
